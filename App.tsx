...
const updateDbfBinary = (dbfBuffer: Uint8Array, features: any[], gridStates: Record<string, number>, buildingStates: Record<string, number>): Uint8Array => {
  ...
  if (fieldsToAdd.length > 0) {
    ...
    const addedRecordSize = fieldsToAdd.length * 1; 
    ...
    for (let r = 0; r < numRecords; r++) {
      ...
      for (let f = 0; f < fieldsToAdd.length; f++) { newBuf[newRecStart + oldRecordLen + f] = 0x20; }
    }
...
const getFeatureStyle = (roadState: number, buildingState: number) => {
  ...
  if (roadState === 1) { fillColor = '#ef4444'; fillOpacity = 0.4; } 
  else if (roadState === 2) { fillColor = '#22c55e'; fillOpacity = 0.4; }
  let color = '#cbd5e1'; 
  let weight = 0.5; 
  let dashArray = '2, 2'; 
  if (buildingState === 1) { color = '#facc15'; weight = 2; dashArray = ''; } 
  else if (buildingState === 2) { color = '#3b82f6'; weight = 2; dashArray = ''; } 
  else if (roadState !== 0) { color = fillColor; weight = 1; dashArray = ''; }
...
export default function App() {
  ...
  const [currentLocation, setCurrentLocation] = useState<MapLocation>(DEFAULT_LOCATION);
  ...
  const stats = useMemo(() => {
      ...
      } else { targetIds = defaultGridFeatures.map(f => f.id); }
      let roadCount = 0; let noRoadCount = 0; let buildingCount = 0; let noBuildingCount = 0;
      targetIds.forEach(id => {
          const r = gridStates[id] || 0; const b = buildingStates[id] || 0;
          if (r === 1) roadCount++; if (r === 2) noRoadCount++;
          if (b === 1) buildingCount++; if (b === 2) noBuildingCount++;
      });
    ...
    try {
      ...
      const cos = new window.COS({
        getAuthorization: (options: any, callback: any) => {
          callback({ TmpSecretId: credentials.secret_id, TmpSecretKey: credentials.secret_key, XCosSecurityToken: credentials.token, SecurityToken: credentials.token, StartTime: credentials.start_time, ExpiredTime: credentials.expire_time });
        }
      });
      const uploadTasks = layers.map(async (layer) => {
        ...
        if (!layer.cloudObjectKey) { layer.cloudObjectKey = targetFile.object_key; layer.cloudFileName = targetFile.filename; }
        ...
        if (geojsonToUpload.features) {
          geojsonToUpload.features.forEach((f: any) => {
            const gs = gridStates[f.id] || 0; const bs = buildingStates[f.id] || 0;
      ...
      if (!silent) { setSaveStatus('success'); setTimeout(() => { setSaveStatus('idle'); setIsSaving(false); }, 2000); }
    } catch (err: any) { console.error("[Cloud] 同步错误:", err); if (!silent) { setSaveStatus('error'); setIsSaving(false); } }
  }, [user, gridStates, buildingStates]);

  const handleManualSave = async () => { if (user) await syncAllToCloud(); };
  const handleAutoSync = useCallback(async (targetUser: User) => {
    setIsSyncing(true); setCustomLayers([]);
    try {
      const syncData = await api.getMyCloudFiles();
      const { files, credentials, cos_info } = syncData;
      if (!files || files.length === 0) { setIsSyncing(false); return; }
      // @ts-ignore
      const cos = new window.COS({
        getAuthorization: (options: any, callback: any) => {
          callback({ TmpSecretId: credentials.secret_id, TmpSecretKey: credentials.secret_key, XCosSecurityToken: credentials.token, SecurityToken: credentials.token, ExpiredTime: credentials.expire_time });
        }
      });
      const downloadAndRender = files.map((file: { filename: string; object_key: string }) => {
        return new Promise<void>((resolve) => {
          cos.getObject({ Bucket: cos_info.bucket, Region: cos_info.region, Key: file.object_key, DataType: 'arraybuffer' }, (err: any, data: any) => {
            if (err) resolve(); else {
              (async () => {
                try { const body = data && (data as any).Body; if (body && (body as any).byteLength > 0) await processFileData(file.filename, body as any, targetUser, file.object_key); }
                catch (e) { console.warn(`[Sync] 跳过文件: ${file.filename}`, e); }
                resolve();
              })();
            }
          });
    ...
    } catch (err) { console.error("[Sync] 失败", err); } finally { setIsSyncing(false); }
  }, []);
  ...
  const processFileData = async (fileName: string, buffer: any, targetUser: User, cloudObjectKey?: string) => {
    ...
    let geojson: any = null; let originalFiles: Record<string, Uint8Array> = {};
    if (fileExt === 'zip') {
        try {
            const JSZipObj = (window as any).JSZip;
            const zip = await JSZipObj.loadAsync(buffer as ArrayBuffer);
            const filePromises: Promise<void>[] = [];
            zip.forEach((relativePath: string, file: any) => { filePromises.push(file.async("uint8array").then((data: Uint8Array) => { originalFiles[relativePath] = data; })); });
            await Promise.all(filePromises);
            if (originalFiles['v_labels.json']) geojson = JSON.parse(new TextDecoder().decode(originalFiles['v_labels.json']));
            else { try { 
                   geojson = await (window as any).shp(buffer); } catch (e) { return; } }
        } catch (e) { return; }
    ...
    if (geojson) {
      ...
      if (polygonsToAdd.length > 0) {
        ...
        const newLayer: ExtendedCustomLayer = { id: newLayerId, userId: targetUser.id, name: fileNameBase + " (可标注)", sourceFileName: fileName, cloudObjectKey, cloudFileName: fileName, type: 'grid', data: { type: 'FeatureCollection', features: polygonsToAdd } as any, timestamp: Date.now(), visible: true, originalFiles, idKeyName: detectedIdKeyName };
        setCustomLayers(prev => [...prev, newLayer]); setGridDataVersion(v => v + 1); setActiveLayerId(newLayerId);
      }
      if (referenceFeatures.length > 0) {
        const newRefLayer: ExtendedCustomLayer = { id: `${userPath}ref_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`, userId: targetUser.id, name: fileNameBase + " (参考层)", sourceFileName: fileName, cloudObjectKey, cloudFileName: fileName, type: 'reference', data: { type: "FeatureCollection", features: referenceFeatures } as any, timestamp: Date.now(), visible: true, originalFiles };
  ...
  const executeDeleteLayer = async () => {
      ...
      const layerId = layerToDeleteId; const layerToDelete = customLayers.find(l => l.id === layerId);
      ...
      if (layerToDelete.cloudObjectKey && layerToDelete.cloudFileName) {
          if (window.confirm("确认删除云端数据吗？")) {
              setIsDeleting(true); try { const delData = await api.deleteCloudFileCredentials(layerToDelete.cloudFileName, layerToDelete.cloudObjectKey); const { credentials, cos_info } = delData;
                  // @ts-ignore
                  const cos = new window.COS({ getAuthorization: (o, cb) => cb({ TmpSecretId: credentials.secret_id, TmpSecretKey: credentials.secret_key, XCosSecurityToken: credentials.token, SecurityToken: credentials.token, ExpiredTime: credentials.expire_time }) });
                  await new Promise<void>((res, rej) => { cos.deleteObject({ Bucket: cos_info.bucket, Region: cos_info.region, Key: layerToDelete.cloudObjectKey }, (err) => err ? rej(err) : res()); });
              } catch (e: any) { alert(`删除失败: ${e.message}`); setIsDeleting(false); return; } finally { setIsDeleting(false); }
          } else { setLayerToDeleteId(null); return; }
      }
      setCustomLayers(prev => prev.filter(l => l.id !== layerId)); if (activeLayerId === layerId) { setActiveLayerId(null); setSelectedFeatureId(null); }
  ...
  const handleGridClick = (e: any) => {
      const id = e.target.feature.id; const { gridStates: cg, buildingStates: cb, labelMode: cm } = stateRef.current;
      let nr = cg[id] || 0; let nb = cb[id] || 0;
      if (cm === 'road') { if (nr === 0) nr = 2; else if (nr === 2) nr = 1; else nr = 0; setGridStates(p => ({ ...p, [id]: nr })); }
  ...
  return (
    <div className="relative w-full h-full bg-slate-900 overflow-hidden text-slate-900 font-sans">
      ...
      <MapContainer center={[DEFAULT_LOCATION.lat, DEFAULT_LOCATION.lng]} zoom={DEFAULT_LOCATION.zoom} className="w-full h-full z-0" zoomControl={false}>
        ...
        <GeoJSON key={`grid-layer-${gridDataVersion}`} ref={geoJsonRef} data={activeGridFeatures as any} style={(f) => { const id = f?.id as string; if (id === selectedFeatureId) return { color: '#ec4899', weight: 4, opacity: 1, fillOpacity: 0.4, fillColor: '#ec4899' }; return getFeatureStyle(gridStates[id] || 0, buildingStates[id] || 0); }} onEachFeature={(f, l) => { const id = f?.id as string; l.on({ click: handleGridClick, mouseover: (e) => { if (id !== selectedFeatureId) e.target.setStyle({ weight: 2, color: '#fff', opacity: 1, fillOpacity: 0.5 }); }, mouseout: (e) => { if (id !== selectedFeatureId) { const { gridStates: cg, buildingStates: cb } = stateRef.current; e.target.setStyle(getFeatureStyle(cg[id] || 0, cb[id] || 0)); } } }); }} />
      </MapContainer>
      ...
      <div className="absolute top-4 left-4 z-[1000] flex flex-col gap-3"> 
        <button onClick={handleManualSave} disabled={isSaving || isDeleting} className="w-12 h-12 flex items-center justify-center bg-white rounded-xl shadow-xl border border-slate-200 text-slate-700 hover:text-emerald-600 transition-all active:scale-95"> 
      ...
      <div className="absolute top-4 right-4 z-[1000] flex flex-col gap-3 items-end"> 
        <div className="bg-white/95 p-2 rounded-2xl shadow-xl border border-slate-100 flex flex-col min-w-[130px] transition-all"> 
          <div className="flex items-center justify-between mb-1">
            ...
            <button 
              onClick={() => { api.logout(); setUser(null); localStorage.removeItem(STORAGE_KEY_USER); }} 
        ...
        <button id="tour-tools-toggle" onClick={() => setIsToolsOpen(!isToolsOpen)} className="bg-white p-3 rounded-xl shadow-xl border flex items-center gap-2 font-bold text-sm hover:bg-slate-50 transition-colors"><Layers className="w-4 h-4 text-emerald-600" />图层</button> 
        {isToolsOpen && ( 
      ...
      {layerToDeleteId && ( <div className="fixed inset-0 z-[3000] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4 text-slate-900"> <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-8 text-center space-y-5"> <AlertTriangle className="w-12 h-12 text-red-500 mx-auto" /> <h3 className="text-xl font-bold">删除确认</h3><p className="text-sm text-slate-500">此操作将永久移除该数据，确认吗？</p> <div className="grid grid-cols-2 gap-3 pt-2"><button onClick={() => setLayerToDeleteId(null)} className="px-4 py-3 rounded-xl border font-bold">取消</button><button onClick={executeDeleteLayer} disabled={isDeleting} className="px-4 py-3 rounded-xl bg-red-500 text-white font-bold">{isDeleting ? '删除中...' : '确认删除'}</button></div> </div> </div> )}
    </div>
  );
}
